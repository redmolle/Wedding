...
const Menu = (props) => {
	...
	const choose = () => {
		props.chooseMeal(props.guest.id, selected, (message) => onSuccess(message), (message) => onInfo(message));
	}
	...
	useEffect(() => {
		...
		const isNeedExec = () => {
			if (meal.length !== selected.length) {
				return true;
			}

			for (var i=0; i<selected.length; i++)
			{
				if (meal[i].dishId !== selected[i]) {
					return true;
				}
			}
...
>
				<Title bold>
					Выберите, что хотели бы попробовать
				</Title>
							...
							<Table className={classes.tableCell}>
								<TableHead>
									<TableRow className={classes.tableHead}>
										...
										<TableCell padding='none'>{category.name.toUpperCase()}</TableCell>
									</TableRow>
								</TableHead>
								<TableBody>
